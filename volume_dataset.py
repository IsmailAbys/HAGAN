...
class Volume_Dataset(Dataset):
    ...
    def __init__(self, data_dir, d='D:/', mode='train', fold=0, num_class=0):
        self.sid_list = []
    ...
    def __getitem__(self, idx):
        img = np.load(self.sid_list[idx])
        class_label = self.class_label_dict.get(self.sid_list[idx], -1) # -1 if no class label
        return img[None,:,:,:], class_label


# if __name__ == "__main__":
#     data =  Volume_Dataset(r"D:\databaseForTraining_after_preprocessing_1024_600_256", fold=0, num_class=0)
#     for batch in data:
#         print(batch[1])
#         break